...
public class WebSocketServerEndpoint {
    ...
    public void onOpen(Session session,  @PathParam("userId") String userId){
        ...
        System.out.println("----[ WebSocket ]---- 用户id为 :" + userId + " 的用户进入WebSocket连接 ! 当前在线人数为 : " + livingSession.size());
    }
    ...
    public void onMessage(String message, Session session, @PathParam("userId") String userId) {
        ...
        System.out.println("--------收到用户id为 :" + userId +" 的用户发送的消息 ! 消息内容为 : " + message);
        System.out.println(message);
        JSONObject jsonObject = JSONObject.parseObject(message);
        Chat msg = JSON.toJavaObject(jsonObject, Chat.class);
        System.out.println(msg);
        sendMessageById(msg.getUsergetid().toString(),message);
